...
mapboxgl.accessToken = 'pk.eyJ1IjoibWlhbWllZHRlY2giLCJhIjoiY2tocXh0NHMwMGViajJ4bWN5NWZxMjFqOCJ9.C44hy16LPWVJlKlX-4Ko5A';
...
function loadImages(callback) {
	...
	colors.forEach(color => {
		window.map.loadImage('images/' + color + '-marker.png', (error, image) => {
			if (error) {
				lastError = error;
			}
...
window.map.on('load', function () {
	loadImages(
		function (error, images) {
			...
			Object.keys(images).forEach(name => {
				window.map.addImage(name, images[name]);
			})
	...
	window.map.on('click', '2018', callback);
	window.map.on('click', '2018 70+%', callback);
	window.map.on('click', '2019', callback);
	window.map.on('click', '2019 70+%', callback);
...
for (var i = 0; i < toggleableLayerIds.length; i++) {
	...
	link.onclick = function (e) {
		...
		var visibility = window.map.getLayoutProperty(clickedLayer, 'visibility');
		...
		toggleableLinks.forEach(link => {
			...
			window.map.setLayoutProperty(link.textContent, 'visibility', isSelected ? 'visible' : 'none');
		});
	};
